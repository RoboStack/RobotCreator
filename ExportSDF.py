    ...
    def Activated(self):
        ...
        for obj in objs:
            if obj.TypeId == "PartDesign::Body":
                ...
                sdfFile.write(
                    "<pose> "
                    + str(pos.Base[0] + com.x)
                    + " "
                    + str(pos.Base[1] + com.y)
                    + " "
                    + str(pos.Base[2] + com.z)
                    + " "
                    + str(pos.Rotation.toEuler()[0])
                    + " "
                    + str(pos.Rotation.toEuler()[1])
                    + " "
                    + str(pos.Rotation.toEuler()[2])
                    + "</pose>\n"
                )
                ...
                sdfFile.write("<mesh>\n")
                sdfFile.write(
                    "<uri>model://" + robotName + "/meshes/" + name + ".stl</uri>\n"
                )
                sdfFile.write("</mesh>\n")
                sdfFile.write("</geometry>\n")
